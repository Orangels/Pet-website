...
const style = {
    ...
    code_part:{
        ...
        // paddingLeft: 5*screen_scale_width
    },
    segmentation_part:{
        borderWidth:2,
...
export default class Md_content_component_EN extends Component{
    // 构造
    constructor(props) {
        super(props);
        // 初始状态
                ...
                case 'title':
                    RenderComponent = (<h1
                        // id={item[key].replace(/ /g,'_')}
                                           key={`${com_key}_${num}`} className={`${className} Doc_title`} style={{ marginBottom:40*screen_scale_width, fontSize:58*screen_scale_width, lineHeight:'30px'}}>{item[key]}</h1>);
                ...
                case 'h3_title':
                    RenderComponent = (
                        <div key={`${com_key}_${num}`} id={item[key].replace(/ /g,'_')} style={style.part_3} className={className}>
                            ...
                            <h3 key={`${com_key}_${num}`} style={{color:'#2B2D2E', fontWeight:1000}}>{item[key]}</h3>
                        </div>
                    );
